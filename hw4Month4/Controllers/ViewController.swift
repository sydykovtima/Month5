...
class ViewController: UIViewController {
    ...
    private func searchProducts(by word: String) {
        ...
        Task {
            do {
                let model = try await NetworkLayer.shared.searchProducts(by: word)
                isLoading = false
                products = model.products
                DispatchQueue.main.async {
                    self.productTableView.reloadData()
...
extension ViewController: UITableViewDelegate {
    ...
    func tableView(
        ...
        let action = UIContextualAction(
            style: .normal,
            title: "Create new product") { _, _, _ in
    ...
    private func deleteProducts(by id: Int) {
        Task {
            do {
                let result = try await NetworkLayer.shared.deleteProductsData(id: id)
                if result {
                    DispatchQueue.main.async {
    ...
    private func askUserConfirmAlert(for indexPath: IndexPath, deleteAction: Bool) {
        if deleteAction {
            let alert = UIAlertController(title: "Вы действительнo хотите удалить?",
            message: "Данное действие нельзя вернуть обратно",
            preferredStyle: .alert)
...
extension ViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView,
        ...
        if collectionView == categoryCollectionView {
            ...
            collectionView == typeOfOrderCollectionView
