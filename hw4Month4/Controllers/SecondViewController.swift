...
class SecondViewController: UIViewController {
    ...
    @IBAction func createProduct() {
        ...
        Task {
            do {
                let result = try await NetworkLayer.shared.postProductsData(model: product)
                if result {
    ...
    private func showSuccessAlert() {
        let alert = UIAlertController(title: "Success", message: "Successfull created product.", preferredStyle: .alert)
